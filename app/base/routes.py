...
def get_prediction():
    ...
    if request.method == 'POST':
        ...
        data = list(map(float, request.get_json().split(',')))  # Get data posted as a json
...
@blueprint.route('/predictform', methods=['POST'])
def get_prediction_form():
	with open('model1.pkl', 'rb') as f:
		model = pickle.load(f)
	if request.method == 'POST':
		data = request.form.to_dict()
		data = list(map(float, data['feature'].split(',')))  # Get data posted as a dict
		data = np.array(data)[np.newaxis, :]  # converts shape from (4,) to (1, 4)
		prediction = model.predict(data)  # runs globally loaded model on the data
		if prediction[0] == 0:
			return render_template('login/resultpass.html')
		else:
			return render_template('login/resultfail.html')
